const availableTestPorts = process.env.NODE_ENV === 'test' 
  ? Array.from({length: 30000}).map((_, i) => 30000 + i)
  : null;

module.exports = rapid => {
  const Configurable = require('./Configurable');
  const isTest = rapid.env === 'test';
  ...
  return class Webserver extends Configurable {
    ...
    constructor() {
      ...
      if(isTest) {
        if(!availableTestPorts.length) throw new Error(`No available test ports`);
        const port = availableTestPorts.shift();
