...
module.exports = rapid => {
  ...
  return class Webserver extends Configurable {
    ...
    async _errorMiddleware(context, next) {
      try {
        ...
        if (rapid.env === 'production' && context.status === 500) {
          context.body = { error: { message: 'Internal server error.', data: error.data } };
        } else {
          context.body = { error: { message: error.message, data: error.data } };
        }
      }
    }
    ...
    async listen() {
      ...
      if(port === 'any' || port === 'auto') { // Find any available port
        resolvedPort = await require('get-port')();
      } else if(Array.isArray(port)) { // Find any available port with a preference for the ports passed
