...
class Rapid {
  ...
  async stop() {
    ...
    if (this.webserver) {
      await this.webserver.stop().catch(error => this.log('Failed to stop webserver: ' + error.message));
    }
    if (this.database) {
      await this.database.stop().catch(error => this.log('Failed to stop database: ' + error.message));;
    }
