...
const validEnvValues = ['test', 'development', 'production'];
const validEnvValuesString = validEnvValues.map(env => `"${env}"`).join(', ');

class Rapid {
  constructor(root, config) {
    this.env = process.env.NODE_ENV || 'development';
    this.isTest = this.env === 'test';
    if (!validEnvValues.includes(this.env)) {
      throw new Error(
        `Invalid NODE_ENV environmental variable "${
          this.env
        }". Valid values: [${validEnvValuesString}]`
      );
    }
  ...
  _resolveRoutes() {
    for (let routesDef of this._addedRoutes) {
      if (isFunction(routesDef)) {
        const result = routesDef(this);
        if (result instanceof this.Router && result !== this.api) {
          this.api.use(result.routes());
          this.api.use(result.allowedMethods());
        }
      }
    }
