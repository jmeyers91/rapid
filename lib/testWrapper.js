    ...
    testFn(description, async () => {
      try {
        ...
        rapid.axios = require('axios').create({
          ...
          validateStatus() { // don't throw for any status
            return true;
          },
        });
        await fn(rapid);
      } finally {
        await rapid.stop().catch(error => console.log(`Failed to stop ${error}`));
        await rapid.database.drop().catch(error => console.log(`Failed to drop ${error}`));
      }
    });
  };
};
